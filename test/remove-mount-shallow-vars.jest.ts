...
import { convertMountShallowVars } from '../src/utils/ast-transformations/individual-transformations/convert-mount-shallow-vars';

describe('convertMountShallowVars', () => {
	const j = jscodeshift.withParser('tsx');

	const transformAndExpect = (source: string, expectedText: string, renderFunction: string, expectedVars: string[]) => {
		const root = j(source);
	...
	it('should convert variable declaration with abstracted function', () => {
		...
		`;
		transformAndExpect(source, expectedText, 'abstractedFunction', ['wrapper']);
	});
	...
	it('should convert assignment expression with abstracted function and remove variable declaration', () => {
		...
		const expectedText = `
			abstractedFunction({ value: 111 });
		`;
		transformAndExpect(source, expectedText, 'abstractedFunction', ['mountedComponent']);
	});
	...
	it('should handle direct variable declaration with shallow function call and variable name as abstracted function', () => {
		...
		const expectedText = `
			shallow(<Component />);
		`;
		transformAndExpect(source, expectedText, 'shallowComponent', ['shallowComponent']);
	});
	...
	`;
		transformAndExpect(source, expectedText, 'abstractedFunction', ['wrapper', 'mountedComponent']);
	});

	it.todo('should handle destructured variable declaration with abstracted function call')
});
